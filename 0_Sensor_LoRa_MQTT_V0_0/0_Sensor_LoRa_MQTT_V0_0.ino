...
// Global variable to store the last time the GPS was updated
unsigned long lastGpsCheck = 0; 
const unsigned long gpsInterval = 200; // Interval in milliseconds
...
void loop() {
  ...
  // Check if 200ms have passed since the last update
  if (currentMillis - lastGpsCheck >= gpsInterval) {
    // Save the current time as the last update time
    lastGpsCheck = currentMillis;

    // Execute your GPS function
    updateGPS(); 
  }
